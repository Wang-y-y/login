var setChry = function () {
        ...
        layui.use('laydate', function(){
            ...
            laydate.render({
                ...
                //range:'-',
                    ...
                    var startTime = ''+(date.hours<10?'0'+date.hours:date.hours)+':'+(date.minutes<10?'0'+date.minutes:date.minutes);
                    // var endTime = ''+(endDate.hours<10?'0'+endDate.hours:endDate.hours)+':'+(endDate.minutes<10?'0'+endDate.minutes:endDate.minutes);
                    bus.minTime = value;
        ...
        layui.use('laydate', function(){
            ...
            laydate.render({
                ...
                min:bus.minTime,
                    ...
                    var startTime = ''+(date.hours<10?'0'+date.hours:date.hours)+':'+(date.minutes<10?'0'+date.minutes:date.minutes);
                    // var endTime = ''+(endDate.hours<10?'0'+endDate.hours:endDate.hours)+':'+(endDate.minutes<10?'0'+endDate.minutes:endDate.minutes);
                    bus.endTime = startTime;
                    bus.Time =bus.startTime+'-'+bus.endTime;
    ...
    var loadMeetingRoom = function () {
        $.ajax({
            ...
            data:{"id":"","device_id":"","roomName":"","roomAddr":"","roomStatus":"","offset":"","pageSize":""},
            dataType:"json",
            success:function (data) {
                ...
                if(data.status == '0'){
                    ...
                    $(".js-addMeetingRoom").empty();
                    $(".js-addMeetingRoom").append('<li data-id="" >-请选择-</li>');
                    data.resp.data.forEach(function (item,index) {
                        $(".js-addMeetingRoom").append($('<li data-id="'+item.id+'">'+item.roomName+'</li>').data("sloutARR",item.slout));
                    });
        ...
        $(".js-addloadMeetingRoom").click(function (e) {
            e.stopPropagation();
            $(".js-addMeetingRoom").show();
        });
        $("body").click(function () {
            $(".js-addMeetingRoom").hide();
        });
        //加载座椅布局
        $(".js-addMeetingRoom").delegate('li','click',function () {

            putExcTemplateUpload();
            var id = $(this).attr("data-id");
            bus.meetingRoomId = id;
            if(id != ""){bus.meetingRoomStatus = 'true';}else{bus.meetingRoomStatus = 'false';}

            var text = $(this).text();
            $(".js-addloadMeetingRoom").text(text);
            ...
            for (var i = 0; i < meetingData.colNum ; i++) {
                ...
                for (var j = 0; j < meetingData.rowNum ; j++) {
                    div += ' <div class="zw js-selectZw js-setSeatNum js-setSelectZw onSelect" data-className="false" data-type="true" data-seatNum = "'+(i+1)+'" data-colNum = "'+(i+1)+'"  data-seatId = "seat'+(i+1)+"-"+(j+1)+'" data-seatId = "seat'+(i+1)+(j+1)+'" data-seatStatus = "seat04" data-seatArea=""></div>';
                }
                var li = '<li>'+div+'</li>';
    ...
    var selectZw = function () {
        $(".js-getSeatNum.onSelect").unbind('click');
        $("body").delegate('.js-getSeatNum.onSelect','click',function () {
            var type = $(this).attr("data-type");
            var keyWord = $(".js-addChryName").val();
            ...
            if(keyWord){
                if(type == "true"){
                    ...
                    if(employeeName && bus.seatNum){
                        ...
                        $(".js-addMeetingChryMenu").append(tr);
                        bus.employeeName = "";
                        bus.company = "";
                        bus.employeeId = "";
                        bus.seatNum = "";
    ...
    var loadEmployeeName = function () {
        ...
        $.ajax({
            ...
            data:{"id":"","employeeName":"","department":"","employeePosition":"","employeeNo":"","phone":"","emailAddr":"","offset":"","pageSize":""},
            dataType:"json",
            success:function (data) {
                ...
                if(data.resp.data){
                    data.resp.data.forEach(function (item,index) {
                        item.pyjc = item.employeeName.spell('first','low');
                        item.pyqc = item.employeeName.spell('low');
                        newRyData.push(item);
                    });
                    data.resp.data.forEach(function (item,index) {
                        $(".js-allRYmenu").append('<li data-company="'+item.company+'" data-employeeId="'+item.id+'">'+item.employeeName+'</li>');
                    });
                }
                ryData = newRyData;
            },
        ...
        $(".js-addChryName").bind("input propertvchange",function (e) {
            e.stopPropagation();
            var keyWord = $(this).val();
            var newkeyWord = keyWord.toLowerCase();
            var ry = ryData.filter(function (item) {
                if(item.employeeName.indexOf(newkeyWord) != -1 || item.pyjc.indexOf(newkeyWord) != -1 || item.pyqc.indexOf(newkeyWord) != -1){
                    return item;
                }
            });
            $(".js-allRY").show();
        ...
        $(".js-allRYmenu").delegate('li','click',function () {
            if(bus.meetingRoomStatus == 'true'){
                ...
                if(isHasChry.length == 0){
                    ...
                    $(".js-addChryName").val(bus.employeeName);
                    $(".js-allRY").hide();
    ...
    var addMeeting = function () {
        $(".js-addMeeting").click(function () {
            var mTitle  = $(".js-addmTitle").val();
            var mTitleArr = mTitle.split("");
            var seTime = bus.dateTime +"#"+bus.Time;
            var mAddr = bus.meetingRoomId;
            var participantsArr = [];
            $(".js-addMeetingChryMenu tr").each(function (index,item) {
                var participantsObj = {};
            ...
            console.log(bus);
//mTitleArr.length<=17
            if(bus.dateTime != undefined  && bus.Time != undefined && mAddr != undefined){
				var sendData=new FormData();
				sendData.append('m_title',encodeURI(encodeURI(mTitle)));
				sendData.append('m_addr',encodeURI(encodeURI(mAddr)));
				sendData.append('seTime',seTime);
				sendData.append('participants',encodeURI(encodeURI(participants)));
				sendData.append('status','');
                $.ajax({
                    type:"post",
                    url:"../../../addMeeting.action",
                    data:sendData,
					processData:false,
                    contentType: false,
                    dataType:"json",
    ...
    /**批量上传开会人员**/
    var putExcTemplate = function () {
        $("#putExcTemplate").click(function () {
            if(bus.meetingRoomStatus == 'true') {
    ...
    var delSeat = function(){
        ...
        $("body").delegate(".js-delSeat","click",function () {
            ...
            $(".js-addMeetingChryMenu tr[data-seatNum='"+seatNum+"']").remove();
            $(".js-setSeatNum[data-seatNum='"+seatNum+"']").removeClass('on');
            $(".js-setSeatNum[data-seatNum='"+seatNum+"']").empty();
            $(".js-setSeatNum[data-seatNum='"+seatNum+"']").addClass("onSelect");
    ...
    /**已添加人数**/
    var addRynum = function () {
        var itemArr = [];
        $(".js-addMeetingChryMenu tr").each(function (index,item) {
            itemArr.push(item);
        });
        $(".js-addRynum").text("已添加："+itemArr.length+"人");
    };
